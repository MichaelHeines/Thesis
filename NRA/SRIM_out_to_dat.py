...
file_name_in = 'easyread/easyread_range_Bi_60realbeam'
file_name_out_p1 = 'dat_out/output_Po_alpha_realbeam60'
...
def srim_out_to_in(a_energy, atomic_number_p1, atomic_number_p2, mfraction, sample_width, save_path, file_name_in, file_name_out_p1, file_name_out_p2):
    ...
    with open(complete_name_in, 'r') as f:
        ...
        for row in reader:
            if -sample_width <float(row[2])<sample_width and -sample_width<float(row[3])< sample_width and float(row[1])<substrate_depth:
                ion = row[0].lstrip("0")
                while len(ion) < 5:
                    ion = "0" + ion
